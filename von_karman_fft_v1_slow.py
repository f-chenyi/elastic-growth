...
fileinfo = open(directory+"INFO_"+str(trial)+".txt",'w')
...
#    w_  = np.zeros([N,N])
    ...
    while error > ERROR_CONV:
        ...
        u11hat_ = 1j*kx*u1hat_
        u12hat_ = 1j*ky*u1hat_
        u21hat_ = 1j*kx*u2hat_
        u22hat_ = 1j*ky*u2hat_
    ...
    #    eps21hat_ = np.fft.fft2(eps21i_) + 0.5*(u12hat_+u21hat_) + 0.5*hat2producthat(w1hat_,w2hat_)
        ...
        what = 1/(D33 + Ef*h**3*k2*k2/12 + xi)*(1j*kx*Nw1hat_ + 1j*ky*Nw2hat_ - D31*u1hat_ - D32*u2hat_ + xi*what_ )
        ...
        w = np.fft.ifft2(what)
        wmag = np.real(w)
        wmax = np.amax(np.amax(wmag))
        
#        error = np.amax(np.amax(wmag))
        
        if wmax > 0.1*h and np.absolute(wmax - wmax_)<1e-5*wmax:# wmax < wmax_ and (wmax_ - wmax)<1e-3*wmax:
            for i in np.arange(N):
                for j in np.arange(N):
                    filew.write(str(np.real(w[i,j]))+'  ')
                filew.write('\n')
            filew.close()
            
            fileinfo.write('Ef: '+str(Ef) + '\n'+'nuf: '+str(nuf) + '\n'+'Es: '+str(Es) + '\n'+
                           'nus: '+str(nus) + '\n'+'h: '+str(h) + '\n'+'dx: '+str(dx) + '\n'+
                           'e11_0: '+str(e11_init) + '\n'+ 'e22_0: '+str(e22_init) + '\n')
            fileinfo.close()
        ...
        print("error:"+ str(error) +", wmax:" + str(wmax))
